function G = spst_GIJ(D, map, options)
%
% G = spst_GIJ(D1, D2, map, options)
%
% Input:    D1, D2  Data matrix with rows = observations, columns=variables
%
...
if (max(size(map))>0)
    d2b_events = dist2border(D1, map);
    d2b_events = d2b_events * ones(1,size(x,2));
end

% For each event calculate distance to nearest neighbor in class 2
dist = nndist(D1, D2, 1, options.distmode);
