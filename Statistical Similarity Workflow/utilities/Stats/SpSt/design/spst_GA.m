...
%           options.maxD:       *4*, any real number
...
if(size(gpi,1) <= nsel)
    error('spst_GA: number of candidates too small!');
end
...
while(iter < options.iter && not(optimized) && no_change_count <=10)
    ...
    % generate 60 children with very low mutation rate
